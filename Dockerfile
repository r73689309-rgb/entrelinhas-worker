# Worker serverless RunPod — ComfyUI + FLUX + PuLID + ReActor
# Os modelos NAO ficam na imagem: vem do network volume em /runpod-volume
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      python3.10 python3.10-dev python3-pip git wget curl ca-certificates \
...
# 3) dependencias dos custom nodes (insightface, reactor, etc.)
